...
function InsertStudent(req, res){
    ...
    var direccion = [];
    var telefono = [];
        if(params.FirstName && params.SecondName && params.Surname && params.SecondSurname
            && params.Date && params.Religion && params.Email && params.Gender && params.Departament 
            && params.Municipality && params.Zone && params.Colony && params.Avenue && params.Street 
            && params.Block && params.HouseNumber && params.Mobile && params.Phone && params.Other){
                student = {
...
function studentUpdate(req,res){
    ...
    var direccion = [];
    var telefono = [];
        Persona.updateMany({_id:studentID}, {$set: 
            {
                FirstName: params.FirstName,
                SecondName: params.SecondName,
                Surname: params.Surname,
        ...
        },(err, studentUpdate)=>
        {
            if (err) {
                ...
                if (!studentUpdate) {
                  res.status(404).send({message: 'No se ha podido actualizar a la student.'});
                }else {
  ...
  function deleteStudent(req, res){
    var studentID = req.params.id;
            ...
            Persona.findByIdAndRemove(studentID,(err, Delete)=>{
                ...
                if(!Delete) return res.status(404).send({message: 'No se ha podido borrar los Datos del Usuario'});
